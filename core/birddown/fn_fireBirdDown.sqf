...
if( !(_veh isKindOf "Air")) exitWith {systemChat "Not in a helicopter";};

//Check if player is gunner or driver
if( ((gunner _veh) != player) && ((driver _veh) != player)) exitWith {hint "Only Pilot or a gunner can use this weapon";}; 
...
if(count _inventory != 2) exitWith { hint "Anti Air is not in the Vehicle Inventory!"; };
_inventory = _inventory select 0;
_inventory = + _inventory; //copy the array and work in copy

//Only one can be used in time
if( _veh getVariable["bird_down_inuse", false] ) exitWith { hint "Anti Air in use"; };
...
switch(_type) do 
{
	case "BDMK1":
	{
        ...
        
		{
			...
			if(__shrt == "antiairfuel" && __count >= 1) then
			{
				_hasweapon = true;
			};
			if(__shrt == "antiairfuelammo") then
			{
				_ammocount = __count;
				...
				{
					_inventory set [_forEachIndex, ["antiairfuelammo", _ammocount - 1] ];
				};
			};
		}
		foreach _inventory;
...
if(_ammocount < 1) exitWith { hint "You do not have ammo for this weapon"; _veh setVariable["bird_down_inuse", false];};
if(!(alive _veh)) exitWith { hint "Vehicle has been hit!"; _veh setVariable["bird_down_inuse", false];};
...
hint parseText format["<img size='2' image='icons\schiessplatz.paa'/>Weapon aimed at %1.", typeOf _target];
...
{	
	...
	hintSilent parseText format["<img size='2' image='icons\schiessplatz.paa'/>Target<br/>%1<br/><br/>Crew<br/>%3<br/>Distance<br/><t size='1.2'>%2m / %4m</t><br/><br/>Height<br/><t size='1.2'>%5m</t>", 
	typeOf _target, 
...
if(!_target_locked) exitWith { hint parseText "<img size='2' image='icons\schiessplatz.paa'/>Targeting Aborted"; _veh setVariable["bird_down_inuse", false];};


 ////////////////////////////////////////////////////////////
 //PHASE 3: Actual Shoot 
  hint parseText "<img size='2' image='icons\schiessplatz.paa'/>Targeting Completed";
 [player,"target_shoot"] call life_fnc_globalSound;
 playSound "target_shoot";
 sleep 1;
 hint parseText "<img size='2' image='icons\schiessplatz.paa'/>Anti Air Deployed!";
...
switch(_type) do 
{
	case "BDMK1":
	{
		...
		{
			_target setFuel 0.02;
			[[0,format["%1 Has been hit by an Anti-Air Fuel Weapon",name player]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		};
	};
	case "BDMK2":
	{
